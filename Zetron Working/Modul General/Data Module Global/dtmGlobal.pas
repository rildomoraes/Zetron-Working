...
uses
  ...
  DB, cxDBData, cxDBExtLookupComboBox, dxSkinsCore, dxSkinBlack, dxSkinBlue,
  ...
  dxSkinsForm, ADODB, IdBaseComponent, IdComponent, IdTCPConnection,
  IdTCPClient, ImgList, Controls, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxClasses, cxControls, cxGridCustomView, cxGrid,
  untIvEncryption, dxSkinDarkRoom, dxSkinFoggy, dxSkinSeven, dxSkinSharp,
  dxSkinSpringTime;
...
type
  ...
  TdmGlobal = class(TDataModule)
    ...
    qryMenuOptions: TADOQuery;
    qryReport: TADOQuery;
    qryCountry: TADOQuery;
    qryProvince: TADOQuery;
    qryCity: TADOQuery;
    qryPrintButtonReport: TADOQuery;
    qryGetTransactionId: TADOQuery;
    qrySetTransactionId: TADOQuery;
    qryCurrencyMaster: TADOQuery;
    qrySetup: TADOQuery;
    qryLogProgram: TADOQuery;
    dxSkinController1: TdxSkinController;
    conReadOnly: TADOConnection;
    qryReportCategory: TADOQuery;
    cgvPrevReportCategory: TcxGridDBTableView;
    dsReportCategory: TDataSource;
    cgvPrevReportCategoryReportCategoryId: TcxGridDBColumn;
    cgvPrevReportCategoryReportCategoryName: TcxGridDBColumn;
    qryGetCounterId: TADOQuery;
    qrySetCounterId: TADOQuery;
    qryInsCounterId: TADOQuery;
    imgIconTabSheet: TImageList;
    stGroup: TcxStyle;
    procedure qryMenuOptionsBeforeOpen(DataSet: TDataSet);
    procedure qryLogProgramBeforeOpen(DataSet: TDataSet);
    procedure qryMenuOptionsBeforePost(DataSet: TDataSet);
    ...
    procedure qryReportCategoryNewRecord(DataSet: TDataSet);
  private
    FInitVCL: TInitVCL;
    FMainMenu: TMainMenu;
    //FReportActionArray: array of TAction;
    FReportMenuItemArray: array of TMenuItem;
    //FDisplayActionArray: array of TAction;
  ...
  public
    ...
    function GetSetupValue(const ASetupId: string; ADefaultValue: variant): variant; overload;
    ...
    function GetCounterId(TableName: string): integer;
    procedure SetCounterId(TableName: string; CounterId: integer);
    procedure InsCounterId(TableName: string);
  end;
...
begin
  TADOQuery(DataSet).Parameters[0].Value := SettingGlobal.ProgramId;
end;
...
procedure TdmGlobal.qryMenuOptionsBeforeOpen(DataSet: TDataSet);
begin
  TADOQuery(DataSet).Parameters[0].Value := SettingGlobal.ProgramId;
end;

procedure TdmGlobal.qryMenuOptionsBeforePost(DataSet: TDataSet);
begin
  DataSet.FieldbyName('ProgramId').AsString := SettingGlobal.ProgramId;
end;
...
begin
  OpenIfClose(qryReportCategory);

  qryReport.Parameters[0].Value := SettingGlobal.ProgramId;
end;
...
procedure TdmGlobal.qryReportCategoryNewRecord(DataSet: TDataSet);
begin
...
begin
  DataSet.FieldbyName('OrderNo').AsInteger := 1;
...
begin
  qrySetup.Close;
  qrySetup.Parameters[0].Value := ASetupId;
  ...
  begin
    qrySetup.Insert;
    if ASetupId = 'GLB01' then // Closing Date
      qrySetup.FieldByName('ProgramId').AsString := ASetupId
    else
...
begin
  qrySetTransactionId.Close;
  qrySetTransactionId.Parameters[0].Value := ATableName;
  qrySetTransactionId.Parameters[1].Value := APrefix;
  qrySetTransactionId.Parameters[2].Value := ATransactionDate;
  qrySetTransactionId.Parameters[3].Value := integer(AUseTotalRun);
  OpenIfClose(qrySetTransactionId);
end;
...
begin
  ...
  qryGetTransactionId.Parameters[0].Value := ATableName;
  qryGetTransactionId.Parameters[1].Value := APrefix;
  qryGetTransactionId.Parameters[2].Value := ALength;
  qryGetTransactionId.Parameters[3].Value := ATransactionDate;
  qryGetTransactionId.Parameters[4].Value := integer(AUseTotalRun);
...
begin
  ...
  qrySetup.Parameters[0].Value := ASetupId;
  if ASetupId = 'GLB01' then // Closing Date
    qrySetup.Parameters[1].Value := ASetupId
  else
    qrySetup.Parameters[1].Value := SettingGlobal.ProgramId;
  OpenIfClose(qrySetup);
  if not qrySetup.IsEmpty then
...
begin
  ...
  FInitVCL.DislpayFormatCurrency := SettingGlobal.OptionCurrencyFormat;
  FInitVCL.DislpayFormatNumeric := SettingGlobal.OptionNumericFormat;}
...
begin
  ...
  qryPositionAccess.Parameters[0].Value := SettingGlobal.LoginPositionId;
  qryPositionAccess.Parameters[1].Value := SettingGlobal.ProgramId;
...
begin
  ...
  qryPrintButtonReport.Parameters[0].Value := AReportId;
  qryPrintButtonReport.Parameters[1].Value := SettingGlobal.ProgramId;
  ...
  begin
    ...
    try
      ...
      //MenuItem.Free; {already handle by parent -> APopupMenu}
    end;
  end;
end;
...
begin
  ...
  SettingGlobal.DisplayFormatLuas := GetSetupValue('GLB11', ',0.00;-,0.00');
  SettingGlobal.DisplayFormatVolume := GetSetupValue('GLB12', ',0.00;-,0.00');
...
procedure TdmGlobal.SetCounterId(TableName: string; CounterId: integer);
begin
  qrySetCounterId.Parameters[0].Value := CounterId;
  qrySetCounterId.Parameters[1].Value := TableName;
  qrySetCounterId.ExecSQL;
end;

procedure TdmGlobal.InsCounterId(TableName: string);
begin
  qryInsCounterId.Parameters[0].Value := TableName;
  qryInsCounterId.ExecSQL;
end;

function TdmGlobal.GetCounterId(TableName: string): integer;
begin
  qryGetCounterId.Close;
  qryGetCounterId.Parameters[0].Value := TableName;
  OpenIfClose(qryGetCounterId);
  if qryGetCounterId.Eof then
  begin
    Result := 1;
    InsCounterId(TableName);
  end
  else
    Result := qryGetCounterId.FieldByName('CounterId').AsInteger;
end;
